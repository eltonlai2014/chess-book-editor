...
| **逐手五級評價＋整局報告（掃描後才出現，全程重用掃描資料、零新後端／不查 positions.db）**：每手依 cp 失分（`aiPlyLoss`，與漏著同一份）分五級 优≤30／好≤80／中≤150／差≤350／劣>350（門檻可調，差/劣 ⊇ 漏著預設200）；`#aiReport`：頂列**紅/黑頁籤切換**（`EDITOR.aiAnalysis.flawSide`，預設 red；`.aiSideTabs`/`.aiSideTab`，active 金底＋該方色字＋標籤帶該方失準數）——選哪方就**只**顯示該方一套（master 定案：分視角＝每方只看自己的，無「全部」）：`aiSideHeadHtml(sc)` 一列＝該方 共N手＋**偏差率**（差/劣占該方手數，= 下方失準清單）＋該方五級 tally（走子方由 `fenSide(points[i-1].fen)` 判、非 i%2，故非標準起手也對；不再帶 紅/黑 tag，頁籤已點名）；下方 `.aiRepGrid`（label↔內容對齊）放**關鍵轉折**（**該方**單手最大失分，≥100 才標）＋**失準手數**（**該方**差/劣 清單，每筆＝步名＋**走後分**（`aiEvalRed`，該手後紅POV分，muted 灰）＋**虧損**（`aiLossTag`→「虧N」/「殺」）＋**建議走法**，點擊經 `navigateTo` 跳手）。轉折與失準的**建議走法**共用 `aiSuggestHtml`「建議 」前綴：該步決策點 `points[idx-1].bestZh` 引擎 best ＋**該建議手的走後分**（`aiEvalRed` 紅POV，＝讀數卡右側同一個 +N；把分綁在各自著法旁，消除「同一手兩種分」誤讀）；muted `.aiRepSug`；翻譯重用 `fillAiBlunderBest`（範圍涵蓋差/劣步）。**建議走法一律顯示**（不綁面板高度——master 要求不可因面板矮而消失）；roomy（`#anBodyAi` clientHeight ≥480，即展開/全畫面）現在**只** gate 次要的**五級評級門檻說明**（`aiThresholdLegendHtml`，由 `AI_GRADE_BANDS` 推導：优≤30／好31–80／中81–150／差151–350／劣>350 + 「每手失分 cp」）。矮則只剩摘要——靠 chart 的 ResizeObserver 重跑 `renderAiView` 響應（面板高由 splitter 決定、穩定，不會回授成迴圈）。走勢圖 hover readout 另綴一顆評級 chip（**膠囊在分數前**）surface 逐手評價（不動左側乾淨棋譜清單）。**殺著去假分**：折算 mate 的損失是 ~30000 級假 cp（會印成「約失 29358 分」），偵測 `point.mate!=null`／`|loss|≥9000` 改印「殺」（`aiMateLoss`/`aiLossTag`，關鍵轉折與失準清單共用同一個 虧N/殺；舊 `aiLossPhrase` 的「約失N分／走入殺局」已併入移除）。配色重用主題感知 `--delta` ramp（优綠…劣紅，無寫死 hex，dark/light 皆驗）。對標 xqpoint 報告但跑**本地引擎**＝無配額/免登入 | `AI_GRADE_BANDS`/`aiGrade`(editor-aichart.js:43/50) ／ `renderAiReport`＋`aiSideHeadHtml`/`aiEvalRed`/`aiSuggestHtml`/`aiMateLoss`/`aiLossTag`(editor-aichart.js，`renderAiView` 末呼叫) ／ readout chip 在 `renderAiReadout`；CSS `.aiGradeChip`/`.aiReport`/`.aiRepGrid`/`.aiSideTabs`/`.aiSideTab`/`.aiFlawEval`/`.aiRepSug`/`.g-*`(editor.css) |
