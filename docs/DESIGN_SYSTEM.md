...
### 設定面板控制項（settings form controls）

`.settingsForm` 的 `<select>`／`number`／`checkbox` 在 [editor.css](../frontend/assets/editor.css)
統一成深色膠囊（原生樣式跟暗主題衝突，是「下拉/數字鈕看起來很醜」的根因）：select
`appearance:none`＋自訂 chevron 背景圖、數字框隱藏原生上下鈕（`appearance:textfield`＋
`::-webkit-*-spin-button` 清掉，鍵盤 ↑/↓ 仍可 increment）、checkbox `accent-color:var(--accent-strong)`。
hover 邊框轉濃、focus 用 `--accent` 邊框＋`rgba(var(--accent-rgb),.18)` box-shadow 環。
**唯一寫死值**＝chevron 顏色 `#a8a094`（＝預設 `--text-muted`，但 CSS var 無法塞進 data-URI SVG
故烤死；所有 UI 主題皆深色，單一靜態色在每個主題都讀得清）。**只 scope `.settingsForm`**，
header（`.boardThemePicker`）與 `.metaForm` 的 select 不動。
