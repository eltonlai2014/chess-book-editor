...
### 表單控制項（select / number / checkbox）

原生 `<select>` 箭頭與 number 上下鈕渲染成 OS 預設（亮、突兀），跟暗色膠囊衝突，是「下拉/數字鈕很醜」的根因。在 [editor.css](../frontend/assets/editor.css) 統一處理：

- **`<select>`（app-wide 基底 `select{}`）**：`appearance:none`＋自訂 chevron 背景圖＋
  `--accent` focus 環，**所有下拉一致**（header 棋盤/介面、設定走子動畫、賽事資訊結果，含 JS 動態建立的）。
  各情境只覆寫 **底色/尺寸** 融入所在表單：`.boardThemePicker`＝`--bg-3` 小膠囊、`.settingsForm`＝
  `--bg-2` 34px 全寬、`.metaForm`＝`--bg-1`＋`--line-strong`（配合 meta 文字框）。
  **⚠️ 陷阱**：情境覆寫底色一律用 `background-color:`，**別用 `background:` 簡寫**——簡寫會把基底的
  chevron `background-image` 清成 none（header/meta 原本正是 `background:` 故箭頭沒套上）。
- **number（`.settingsForm`）**：隱藏原生上下鈕（`appearance:textfield`＋清 `::-webkit-*-spin-button`，
  鍵盤 ↑/↓ 仍可 increment）。
- **checkbox（`.settingsForm`）**：`accent-color:var(--accent-strong)`。
- **唯一寫死值**＝chevron 顏色 `#a8a094`（＝預設 `--text-muted`，但 CSS var 無法塞進 data-URI SVG
  故烤死；所有 UI 主題皆深色，單一靜態色在每個主題都讀得清）。
